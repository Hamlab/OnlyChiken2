...
to setup
  ...
  set burned-trees 0
  set allpixels (count patches with [pcolor = yellow ]) +  (count patches with [ pcolor = blue ]) + (count patches with [pcolor = orange ]) + (count patches with [pcolor = green ])
  set allbrown ((count patches with [pcolor = brown ]) / 63000 ) * 100
  set Pfort 80
  set Pmoyen 60
  set Pfaible 40
  set Ptresfaible 20
...
to go
  ...
  ask fires
    ...
    [ifelse (pcolor = yellow ) [if (random-float 100) < Pfort [ignite]]
      [ifelse (pcolor = green ) [if (random-float 100) < Pmoyen [ignite ]]
        [ifelse (pcolor = blue ) [if (random-float 100 ) < Pfaible [ignite ]]
          [if (pcolor = orange  ) [if (random-float 100) < Ptresfaible [ignite ]]]]]]
...
to setup-all
  ...
  ;create-moutons Animal
  ;ask moutons [
   ; set shape "sheep"
    ;set size 6
    ;set color white
    ;setxy random-xcor random-ycor
    ;set vie 50 + random 50 ]
...
;; i représente le % d'arbres marron qui reste en soustrayant les 20 % d'arbres fort des 87 % des patches marron initial
...
;
;to check-mouton
;  ask moutons [ if xcor == ask turtles with [ color = red or color = red - 0.3 or color = red - 0.6 or color = red - 0.9 ]
